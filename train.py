...
import pickle
import smtplib
from email import encoders #// this is for encode the payloads for transport through compliant mail servers
...
def TakeImages():
    Id=(txt.get())#يجيب ال id
    name=(txt2.get()) #يجيب الاسم
    if(is_number(Id) and name.isalpha()): #اذا الايدي رقم و اذا الاسم حروف ...يدخل بهذة الشرط
        ...
        while(True):
            ...
            elif sampleNum>60:# يخزن 60 صورة ويطلع
                break
        cam.release()
        cv2.destroyAllWindows()
        res = "Images Saved for ID : " + Id +" Name : "+ name # يطلع ع البلوك مال النوتفميشن
        row = [Id , name]
        with open('StudentDetails\StudentDetails.csv','a+') as csvFile: # يفتح فايل ال csv و يخزن بي الاسم و الاايدي
...
def TrackImages():
    ...
    while True:
        ...
        for(x,y,w,h) in faces:
            ...
            if(conf < 50):
                ...
                attendance.loc[len(attendance)] = [Id,tt,date,timeStamp]            # هنا نقص ب ال aa افتح الكيت و جيمة
    ...
    email_sender = 'gazproject2019@gmail.com'
    email_receiver = 'sabbarzaid@gmail.com'
